...
public class LimpiezaDao {
	...
	public int agregar(Limpieza objeto) {		
		int id = 0;
		try {
	...
	public void actualizar(Limpieza objeto) throws HibernateException {
		try {
			iniciaOperacion();
	...
	public void eliminar(Limpieza objeto) throws HibernateException {
		try {
			iniciaOperacion();
	...
	public Limpieza traerLimpieza(long idLimpieza) throws HibernateException {
		Limpieza objeto = null;
		try {
			iniciaOperacion();
			objeto = (Limpieza) session.get(Limpieza.class, idLimpieza);		
		/*} catch (HibernateException he) {
			manejaExcepcion(he);
	...
	public List<Limpieza> traerLimpieza() throws HibernateException {
		List<Limpieza> lista=null;
		try {
			iniciaOperacion();
			lista=session.createQuery("from Limpieza c order by c.idLimpieza asc c.limpia asc").list();
	} finally {
		session.close();
	}
	return lista;	
