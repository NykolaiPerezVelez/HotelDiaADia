...
public class ClienteDao {
	...
	public Cliente traerClienteYFacturas(long idCliente) throws HibernateException {
		Cliente objeto = null;
		try {
			iniciaOperacion();
	...
	public Cliente traerClienteYEstadias(long idCliente) throws HibernateException {
		Cliente objeto = null;
		try {
			iniciaOperacion();
			String hql="from Cliente c where c.idCliente ="+idCliente;
			objeto=(Cliente) session.createQuery(hql).uniqueResult();
			Hibernate.initialize(objeto.getEstadias());
		}
		finally {
			session.close();
		}
